...
const userController = {
    ...
    login : async (req,res)=> {
        ...
        if(!currentUser){
            return res.status(404).json({errors : {email : "El email ingresado no esta registrado"}})
        }
        const bcryptResponse = await bcrypt.compare(password, currentUser.password)

        if(!bcryptResponse){
            return res.status(404).json({errors : {password : "Las credenciales ingresadas no son validas"}})
        }
        ...
        jwt.sign(saveToken,SECRET, {expiresIn : "60m"}, /*Callback */ (err,token)=> {
                return res.status(201).json({token})
            })
